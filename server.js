...
app.post('/cadUsuario', (req, res) => {
  const { nome_completo, idade_usuario, user_pessoa, email_usuario, senha_usuario } = req.body;

  if (!nome_completo || !idade_usuario || !user_pessoa || !email_usuario || !senha_usuario) {
    return res.status(400).json({ error: 'Dados incompletos' });
  }

  const sql = 'INSERT INTO pessoa (nome_completo, idade_usuario, user_pessoa, email_usuario, senha_usuario) VALUES (?,?,?,?,?)';
  db.query(sql, [nome_completo, idade_usuario, user_pessoa, email_usuario, senha_usuario], (err, result) => {
    if (err) {
